...
## 8. 렌더링 품질 고도화 및 프리셋 시스템 (v2.1 Update)

### [문제점]
- **하이라이트 백화 현상**: 하이라이트가 입술 색상을 하얗게 덮어버려("Milky"), 색감이 탁해지는 문제.
- **하이라이트 형태**: 윗입술 하이라이트가 너무 가늘게 표현됨.
- **질감 품질**: 하이라이트가 자글자글하게 깨지는(Pixelated) 현상 발생.
- **사용성**: 매번 복잡한 파라미터를 조절해야 하는 불편함.

### [수행 작업 및 결과]
1.  **스펙큘러 독립 블렌딩 (Better Blending)**
    -   `Screen`, `Add`, `Overlay`, `Color Dodge` 등 독립적인 블렌딩 모드를 도입했습니다.
    -   특히 **Color Dodge** 모드는 입술 본연의 색을 투명하게 비추며 고급스러운 광택을 만들어냅니다.

2.  **하이라이트 두께 제어 (Highlight Thickness)**
    -   `UPPER_WIDTH`, `LOWER_WIDTH` 슬라이더를 추가하여 하이라이트의 세로 폭을 조절할 수 있게 되었습니다.
    -   기본값을 2.5배 상향하여 더욱 도톰한 볼륨감을 연출했습니다.

3.  **텍스처 스무딩 (Surface Smoothing)**
    -   Normal Map 생성 전 단계에 **Denoising** 로직을 추가하여, 미세 픽셀 노이즈를 제거했습니다.
    -   이를 통해 액체의 표면 장력과 같은 **매끄럽고(clean) 촉촉한(moist)** 물광 질감을 구현했습니다.

4.  **스타일 프리셋 및 퀵 세팅 (Quick Style)**
    -   **Glossy**, **Satin**, **Matte**의 3가지 핵심 제형을 정의하고 전용 JSON 프리셋을 구축했습니다.
    -   UI 상단에 퀵 버튼을 배치하여 원클릭으로 최적화된 파라미터(블렌딩, 두께, 스무딩 등)를 적용할 수 있습니다.

5.  **파라미터 최적화 (UI Simplification)**
    -   불필요한 `Light Direction` 슬라이더를 제거하고 표준 조명으로 고정했습니다.
    -   `Spec Strength` 최대값을 제한하여 테두리 역전 현상을 방지했습니다.
